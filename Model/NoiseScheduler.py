...
class NoiseScheduler:
    ...
    def add_noise(self, original_image, noise, t):
        ...
        sqrt_alpha_cum_prod = self.sqrt_alpha_cum_prod[t].to(original_image.device).reshape(batch_size)
        sqrt_one_minus_alpha_cum_prod = self.sqrt_one_minus_alpha_cum_prod[t].to(original_image.device).reshape(batch_size)

        for _ in range(len(original_shape) - 1):
            sqrt_alpha_cum_prod = sqrt_alpha_cum_prod.unsqueeze(-1)
            sqrt_one_minus_alpha_cum_prod = sqrt_one_minus_alpha_cum_prod.unsqueeze(-1)
        
        return (sqrt_alpha_cum_prod.to(original_image.device) * original_image 
                + sqrt_one_minus_alpha_cum_prod.to(original_image.device) * noise)
